...
namespace GorgleDevs.Wpf.Samples.DesignCanvas
{
	public class Guidelines : IEnumerable<Guideline>
	{
		...
		public double SnapVerticalEdge(double position)
		{
			return SnapEdge(position, GuideLineOrientation.Vertical);
		}

		public double SnapHorizontalEdge(double position)
		{
			return SnapEdge(position, GuideLineOrientation.Horizontal);
		}

		private double SnapEdge(double position, GuideLineOrientation orientation)
		{ 
			var results = new List<SnapResult>();

			foreach (var guideline in _guidelines.Where(gl => gl.Orientation == orientation && gl.Type == GuidelineType.Edge))
			{
				guideline.IsActive = false;
				var delta = position - guideline.Position;
				if(_inSnapDistance(delta))
				{
					results.Add(new SnapResult
					{
						GuideLine = guideline,
						Delta = delta,
						SnappedRect = new Rect(guideline.Position, 0, 0, 0)
					});
				}
			}
			...
			if (results.Any())
			{
				...
				return closest.SnappedRect.Left;
			}
